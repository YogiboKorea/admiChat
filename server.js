...
async function updateMarketingConsent(memberId) {
  ...
  const payload = {
    shop_no: 1,
    marketing: {
      sms_agree:   'T',   // SMS 수신동의
      email_agree: 'T'    // 이메일(뉴스메일) 수신동의 – 필요 없으면 제거 가능
    },
    // 선택 항목: privacy_consent(마케팅 목적 동의)도 같은 API에서 같이 올려줍니다.
    // 실제 파라미터 이름은 'marketing_consent' 으로 내부 매핑됩니다.
    marketing_consent: 'T'
  };

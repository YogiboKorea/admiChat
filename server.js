...
//      ✅ 수정: "이벤트 페이지(1_promotion.html)에서만 시작" 규칙 제거
//      ✅ 추가: yogibo.kr 도메인 유입이면 어떤 페이지로 시작해도 로그 저장
// ==========================================================
app.post('/api/trace/log', async (req, res) => {
  try {
    ...
    console.log('[LOG] 요청:', {
      ...
      prevUrl: prevUrl?.substring(0, 80),
      userIp,
      isDev
    });
    ...
    if (visitorId) {
      ...
      if (lastLog) {
        ...
        if (timeDiff < SESSION_TIMEOUT) {
          isNewSession = false;
          isRevisit = lastLog.isRevisit || false; // inherit
        } else {
          isNewSession = true;
    ...
    // --------------------------------------------------------
    // 6. ✅ 진입점(Entry Point) 규칙 변경
    //   - 기존: 신규 세션이면 1_promotion.html에서만 시작 허용
    //   - 변경: "yogibo.kr 도메인 유입"이면 어떤 페이지로 시작해도 허용
    //   - (옵션) yogibo.kr 아닌 외부/스킨은 계속 제외 가능
    // --------------------------------------------------------
    // 스킨/관리 페이지 등 제외(기존 유지)
    if (currentUrl && currentUrl.includes('skin-skin')) {
      return res.json({ success: true, msg: 'Skin Ignored' });
    }

    // ✅ 도메인 판별(상대경로도 있을 수 있으니 방어적으로)
    const isYogiboEntry =
      (typeof currentUrl === 'string' && /yogibo\.kr/i.test(currentUrl)) ||
      (typeof prevUrl === 'string' && /yogibo\.kr/i.test(prevUrl)) ||
      (typeof currentUrl === 'string' && currentUrl.startsWith('/')) ||
      (typeof prevUrl === 'string' && prevUrl.startsWith('/'));

    // 원하시면 외부 도메인에서 들어오는 첫 로그는 컷(데이터 오염 방지)
    // → 단, 지금 니즈가 "직접/기타도 결제 전환 제대로"이므로
    //    yogibo에서 시작만 했으면(= isYogiboEntry) 저장하도록 합니다.
    if (!isYogiboEntry) {
      return res.json({ success: true, msg: 'Not yogibo entry' });
    }
...
// ==========================================================
// [API 3] 방문자 목록 조회 (searchId 필드 추가)
// ==========================================================
...
// ==========================================================
// [API 5] 퍼널 분석
//      ✅ 수정 포인트: "직접/기타"도 이벤트페이지 시작 강제 X
//      ✅ 유효 방문자(validVisitors) 조건을 "yogibo.kr 유입" 기반으로 완화
// ==========================================================
app.get('/api/trace/funnel', async (req, res) => {
  try {
    ...
    // ✅ 유효 방문자: 기간 내 yogibo.kr 내부 페이지 히트가 1번이라도 있으면 포함
    // (상대경로로 저장되는 로그도 있을 수 있어서 "/" 시작도 포함)
    const validVisitors = await db.collection('visit_logs1Event').distinct('visitorId', {
      createdAt: dateFilter,
      $or: [
        { currentUrl: { $regex: /yogibo\.kr/i } },
        { currentUrl: { $regex: /^\// } }
      ]
    });
      ...
      },
      {
        $group: {
          ...
          step2_visitors: {
            $addToSet: {
              $cond: [
                { $regexMatch: { input: "$currentUrl", regex: "product|detail.html" } },
                "$uniqueId",
                "$$REMOVE"
              ]
            }
          ...
          step3_visitors: {
            $addToSet: {
              $cond: [
                { $regexMatch: { input: "$currentUrl", regex: "basket.html" } },
                "$uniqueId",
                "$$REMOVE"
              ]
            }
          ...
          step4_visitors: {
            $addToSet: {
              $cond: [
                { $regexMatch: { input: "$currentUrl", regex: "orderform.html" } },
                "$uniqueId",
                "$$REMOVE"
              ]
            }
          ...
          step5_visitors: {
            $addToSet: {
              $cond: [
                { $regexMatch: { input: "$currentUrl", regex: "order_result.html" } },
                "$uniqueId",
                "$$REMOVE"
              ]
            }
...
// ==========================================================
// [API 7] 섹션 클릭 로그 저장 (visitorId 저장 추가)
// ==========================================================
...
// ==========================================================
// [API] 특정 버튼 클릭 사용자 조회 (클릭 DB 직접 조회로 누락 방지)
// ==========================================================
...
// ==========================================================
// [API 11] 특정 채널로 유입된 방문자 목록 조회 (First Touch 적용)
// ==========================================================
    ...
    const pipeline = [
      ...
      },
      {
        $group: {
          ...
          count: { $sum: 1 },
          channelHistory: { $push: "$computedChannel" }
        }
      },
      {
        $addFields: {
      ...
      { $match: { representativeChannel: channelName } },
      {
        $project: {
          _id: 0,
          searchId: "$_id",
